...
function pick(x, y) {
  ...
  for (const h of hits) {
    ...
    if (!r) continue;
    /* a wall or a tree in front of it blocks the click */
    const occ = app.place?.occluders?.length ? ray.intersectObjects(app.place.occluders.filter(shown), true) : [];
...
export function buildInteract() {
  ...
  el.addEventListener('pointermove', e => {
    if (e.pointerType !== 'mouse' || app.mode === 'game' || app.mode === 'scripted') return;
  ...
  el.addEventListener('pointerup', e => {
    if (!down || app.mode === 'game' || app.mode === 'scripted') return;

...
/* The hour of the day on the scene's date, at the scene's place. The sun and the moon are where they really were;
   one directional light is the sun by day and the moon by night. Everything that is not the ground takes one
   more factor for the sun: the hills' shadow and the clouds' shadow where you stand (objects.sunVis). */
...
/* sunVis: the sun's share where you stand */
export const objects = { sunVis: { value: 1 } };
/* modules that read the horizon maps (the trees) follow the sun's sector: fn(i0, i1) */
export const horizonHooks = new Set();
const L = {};
...
function updateLighting(dt) {
  ...
  const [y, m, d] = P.date;
  const jd = julian(localToUTC(y, m, d, tod.hour, P.tz ?? -5, P.dst ?? true));
  ...
  for (const h of horizonHooks) h(sector.i0, sector.i1);
  ...
  /* exposure: brighter at night so the moonlight reads, darker at noon; a scene can lift it (app.exposureK) */
  app.exposureBase = (P.exposure ?? 1) * lerp(1, P.nightExposure ?? 2.6, night) * lerp(1, 1.25, tod.dim * (1 - night));
  app.renderer.toneMappingExposure = app.exposureBase * (app.exposureK ?? 1);
  ...
  if (envDirty || Math.abs(key - lastEnv) > (tod.playing ? .25 : .04)) {
    ...
    /* the ground's bounce: snow gives back most of the light */

...
import { kramer } from '../cast/kramer.js';
...
export function photo() {
  ...
  g.fillText('THE APARTMENT', pad, H + pad + cap / 2 + pad * .2);
  ...
  g.fillText(`APT 5A · 129 WEST 81ST STREET · ${clockText(tod.hour)}`, c.width - pad, H + pad + cap / 2 + pad * .2);
  c.toBlob(b => {
    const a = document.createElement('a'); a.href = URL.createObjectURL(b); a.download = 'the-apartment.png';
...
export function buildUI() {
  ...
  /* Kramer */
  $('#kramer').addEventListener('click', () => kramer.trigger());
  ...
  /* keyboard */
  const VIEW_KEYS = { Digit1: 'overview', Digit2: 'audience', Digit3: 'plan', Digit4: 'door', Digit5: 'kitchen' };
  addEventListener('keydown', e => {
    ...
    else if (e.code === 'KeyK') kramer.trigger();

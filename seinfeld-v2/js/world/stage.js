...
export function buildStage() {
  ...
  {const grid = new THREE.Group(); root.add(grid); G.grid = grid; const pipe = mat(0x2a2a2d, .5, .6), Y = 5.4;
   for (let z = -9; z <= 6.01; z += 3) { const p = cyl(grid, .035, .035, 17, pipe, 0, Y, z, 10); p.rotation.z = Math.PI / 2; }
   for (const x of [-8.5, -3, 3, 8.5]) { const p = cyl(grid, .035, .035, 15.2, pipe, x, Y, -1.5, 10); p.rotation.x = Math.PI / 2; }
   for (const x of [-8.5, 8.5]) for (const z of [-9, 6]) cyl(grid, .008, .008, 4, mat(0x55555a, .4, .8), x, Y + 2, z, 6);
   const aims = [[-6, -8, -3.5, -1.6], [-3, -8, -1.5, -2.6], [0, -8, .3, -2.4], [3, -8, 2.2, -1.8], [6, -8, 3.8, -.8],
     [-6, -3, -4.2, .2], [-3, -3, -2, .6], [0, -3, -.4, -1.4], [3, -3, 2.4, .2], [6, -3, 4.6, .6],
     [-5, 3, -2.6, .9], [-1.5, 3, -.8, -.8], [1.5, 3, 1.4, -1.2], [5, 3, 3.6, .2]];
   G.beams = new THREE.Group(); root.add(G.beams);
   aims.forEach(([x, z, ax, az], i) => {
     const pos = V(x, Y - .02, z), aim = V(ax, .6, az);
     fresnel(grid, pos, aim);
     if (i % 3 === 1 || i >= 10) G.beams.add(beam(V(x, Y - .3, z), aim, .1, 1.1));
   });}

  G.spots = [];
  for (const [p, t, i] of [[V(-3.2, 5.3, 5.6), V(-1.6, .7, .4), 34], [V(3.4, 5.3, 5.8), V(2.4, .8, -.6), 30], [V(0, 5.3, 1.5), V(-.5, .6, -2.2), 18]]) {
  ...
  {const jacks = new THREE.Group(); root.add(jacks); const wood = mat(0xc9a96e, .8), bag = mat(0x4a4a3a, .95);
   for (const w of walls) {
     if (!w.g || w.len < 1 || w.H < 2 || ![W.win, W.east, W.se, W.west, W.bedW, W.north, W.bathE, W.corE, W.bedS].includes(w)) continue;
   ...
   decal('JERRY APT · FLAT 3', W.win, 2.0, 1.7);
   decal('KITCHEN · FLAT 7', W.east, 1.3, 1.7);
   decal('BEDRM · FLAT 12', W.north, 3.0, 1.7);
   G.jacks = jacks;}

  /* ---------- bleachers + audience ---------- */
  {const bl = new THREE.Group(); root.add(bl); G.bleachers = bl; const rnd = seeded(33);
   const rows = 6, z0 = 7.4, depth = 1.0, rise = .42, halfW = 8;
   for (let i = 0; i < rows; i++) box(bl, halfW * 2, rise * (i + 1), depth, mat(0x2a2a30, .9), 0, rise * (i + 1) / 2 - .385, z0 + i * depth + depth / 2);
   for (let i = 0; i < rows; i++) box(bl, halfW * 2, .03, .06, mat(0x8a8a8e, .5, .3), 0, rise * (i + 1) - .37, z0 + i * depth + .03);
   const seats = [], people = [];
   for (let i = 0; i < rows; i++) for (let x = -halfW + .45; x < halfW - .3; x += .62) {
  ...
  {const cams = new THREE.Group(); root.add(cams); G.cams = cams; const dark = mat(0x2b2c30, .5, .3), grey = mat(0x6b6d72, .5, .5);
   [[-4.6, 5.6], [-1.5, 6.1], [1.7, 6.1], [4.8, 5.6]].forEach(([x, z], i) => {
     ...
     c.lookAt(0, -.385, -1.6); c.rotateY(Math.PI);
     studio.cams.push({ group: c, tally });
   });}

  /* ---------- boom mic ---------- */
  {const b = new THREE.Group(); root.add(b); G.boom = b; const grey = mat(0x55575c, .45, .6);
   for (let k = 0; k < 3; k++) { const a = k * Math.PI * 2 / 3; tube(b, V(6.6, 1.2, 3.9), V(6.6 + Math.cos(a) * .5, -.38, 3.9 + Math.sin(a) * .5), .02, grey); }
   tube(b, V(6.6, -.38, 3.9), V(6.6, 2.7, 3.9), .03, grey);
   tube(b, V(7.4, 2.95, 4.4), V(2.2, 3.05, .5), .022, grey);
   cyl(b, .035, .035, .3, MAT.black, 2.2, 2.85, .5);
   sph(b, .07, mat(0x8a8a8a, .95), 2.2, 2.68, .5, 12, 10).scale.set(1, 1.4, 1);}
  ...
  {const s = new THREE.Group(); root.add(s); G.sign = s; const t = TX.applauseTex();
   ...
   s.position.set(0, 6.3, 6.8);}

  /* tape marks on the floor */
  {const marks = new THREE.Group(); root.add(marks);
   [[-2.2, 4.4, 0xe0c030], [.2, 4.6, 0xd03a3a], [2.5, 4.3, 0x3a8ad0], [-.9, 2.9, 0xe0c030]].forEach(([x, z, c]) => {
...
function updateStage(dt) {
  ...
  G.grid.position.y = (1 - k) * 7; G.beams.position.y = (1 - k) * 7; G.sign.position.y = 6.3 + (1 - k) * 6;
...
function restoreSkins(on) {
  ...
  MAT.wallSkin.map = on ? plywood : null; MAT.wallSkin.color.setHex(on ? 0xffffff : 0xf7f6f2); MAT.wallSkin.normalMap = on ? null : MAT.upper.normalMap; MAT.wallSkin.needsUpdate = true;

...
import { tv, tvLevel } from './screens.js';

/* Time of day. The sun keeps coming through the window (the model's "north" is
   free), rising and falling with the hour; at night it becomes a blue moon.
...
export function buildLighting() {
  ...
  const sc = L.sun.shadow.camera; sc.left = -12.5; sc.right = 12.5; sc.top = 12.5; sc.bottom = -12.5; sc.near = 1; sc.far = 60;
  ...
  L.fill = new THREE.DirectionalLight(0xeaf0ff, .28); L.fill.position.set(9, 7, 11); s.add(L.fill);
  L.kitchen = new THREE.PointLight(0xffe6c0, 4, 6.5, 2); L.kitchen.position.set(3.1, 2.25, -.5); app.root.add(L.kitchen);
  L.ceiling = new THREE.PointLight(0xffeedd, 2.2, 10, 2); L.ceiling.position.set(-.5, 2.5, .2); app.root.add(L.ceiling);
  L.corridor = new THREE.PointLight(0xffe2b0, 2.5, 4.5, 2); L.corridor.position.set(4.3, 2.0, -3.9); app.root.add(L.corridor);
...
function updateLighting(dt) {
  ...
  const d = look.dim;
  const lampOn = app.lamp?.on !== false;
  if (app.lamp) { app.lamp.light.intensity = lampOn ? lerp(6, 9, d) : 0; app.lamp.shadeMat.emissiveIntensity = lampOn ? lerp(6, 8, d) : .15; }
  L.ceiling.intensity = lerp(2.2, 1.4, d);
  L.kitchen.intensity = lerp(4, 5.2, d);
  L.corridor.intensity = lerp(2.4, 3.2, d);
  if (MAT.sconce) MAT.sconce.emissiveIntensity = lerp(5.5, 7, d);
  if (app.bedBlinds) app.bedBlinds.emissiveIntensity = lerp(.28, .02, d);
  if (tv.light) {
    const n = tvLevel(app.time);
    tv.light.color.setHex(tv.channel.light || 0x000000);
    tv.light.intensity = 2.2 * n + 2.6 * n * d;

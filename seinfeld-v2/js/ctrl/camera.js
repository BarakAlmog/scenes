...
import { MAIN_POLY } from '../world/apartment.js';
...
const FOOT = corners(-6.1, 6.35, -7.9, 3.45, [0, 2.7]);
const MAIN_PTS = MAIN_POLY.flatMap(([x, z]) => [V(x, 0, z), V(x, 2.3, z)]);
const MAIN_MID = MAIN_PTS.filter(p => Math.abs(p.x) < 4.4);

export const VIEWS = {
  overview: { label: 'Overview', dir: V(6.4, 5.4, 9.6).normalize(), target: V(.15, .2, -2.1), pts: FOOT, margin: .9 },
  audience: { label: 'Audience', dir: V(0, .1, 1).normalize(), target: V(.3, 1.05, -1.0), pts: MAIN_PTS, portraitPts: MAIN_MID, margin: .97 },
  plan: { label: 'Floor plan', dir: V(0, 1, .02).normalize(), target: V(.15, 0, -2.2), pts: corners(-6.1, 6.35, -7.9, 3.45, [2.7]), margin: .94 },
  door: { label: 'Front door', pos: V(.15, 1.72, 1.75), target: V(1.55, 1.12, -3.1) },
  kitchen: { label: 'Kitchen', pos: V(4.55, 1.68, .35), target: V(-1.4, .95, -.6) },
  studio: { label: 'Studio', dir: V(.62, .5, 1).normalize(), target: V(0, 1.2, 0), pts: corners(-8.6, 8.6, -9.5, 13.4, [-.4, 5.6]), margin: .98, hidden: true },
};
...
function fit(view) {
  ...
  const pts = c.aspect < 1 && view.portraitPts ? view.portraitPts : view.pts;
  let lo = 1, hi = 120;
  for (let i = 0; i < 26; i++) {
    const d = (lo + hi) / 2;
    cam.position.copy(view.target).addScaledVector(view.dir, d); cam.lookAt(view.target); cam.updateMatrixWorld();
    let ok = true;
    for (const p of pts) { tmp.copy(p).project(cam); if (Math.abs(tmp.x) > view.margin || Math.abs(tmp.y) > view.margin || tmp.z > 1 || tmp.z < -1) { ok = false; break; } }
...
export function resolve(name) {
  ...
  return { pos: v.target.clone().addScaledVector(v.dir, fit(v)), target: v.target.clone() };
}
...
export function flyTo(target, dur = 1.6) {
  ...
  if (typeof target === 'string') { view.name = target; view.moved = false; }
...
export function updateCamera(dt) {
  ...
  if (app.mode === 'orbit') {
    ...
    ctl.update();
    const t = ctl.target, zMax = app.stage ? 12 : 4;
    const cx = clamp(t.x, -7.5, 7.5), cy = clamp(t.y, -.2, 3.2), cz = clamp(t.z, -9.5, zMax);

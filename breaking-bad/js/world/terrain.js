...
/* The ground: three square rings of 257 x 257 heights (near +-512 m at 4 m, mid +-2048 m at 16 m, far +-8192 m
   at 64 m) and a polar skyline out to 60 km. Each finer ring blends into the next at its edge, so they meet.
   A GPU pass marches every texel toward 16 compass points and keeps the highest horizon angle: the ground
   shader turns that into shadows of the mesas at any hour, and into sky light (ambient occlusion). */
...
export function horizonAt(T, x, z, lift = 1.5) {
  ...
  for (let k = 0; k < DIRS; k++) {
    ...
    let best = -.6, t = 2;
    for (let s = 0; s < 96 && t < 60000; s++, t *= 1.1) {
...
uniform float uHasSky, uRing, uDir0, uREff, uLift;
...
export function computeHorizon(T) {
  ...
  const mat = new THREE.ShaderMaterial({ vertexShader: HOR_VS, fragmentShader: HOR_FS, depthTest: false, depthWrite: false,
    uniforms: { tR0: { value: ringTex[0] }, tR1: { value: ringTex[1] }, tR2: { value: ringTex[2] }, tR3: { value: ringTex[3] }, tSky: { value: skyTex },
      ...
      uHasSky: { value: s ? 1 : 0 }, uRing: { value: 0 }, uDir0: { value: 0 }, uREff: { value: R_EFF }, uLift: { value: .4 } } });

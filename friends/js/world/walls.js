...
import { MAT, PAINT } from '../lib/materials.js';
import { smooth, keep } from '../lib/util.js';

export const WALL_H = 2.75, STUB_H = .14;
export const walls = [];
export const blockers = [];   /* solid wall runs for walk-mode collision: [ax, az, bx, bz] */

/* [y0, y1, key, extra thickness] from the floor up. A key in PAINT is a flat colour on the shared
   vertex-colour paint material, so a wall costs a few draw calls; any other key names a material. */
const LAYERS = {
  monica: [[0, .12, 'mBase', .012], [.12, 2.5, 'mPurple', 0], [2.5, 2.55, 'mCrown', .03], [2.55, 99, 'mTop', 0]],
  brick: [[0, 99, 'brick', 0]],
  brickOut: [[0, 99, 'brickOut', 0]],
  green: [[0, .12, 'mBase', .012], [.12, 99, 'pine', 0]],
  mbath: [[0, 1.25, 'tileW', 0], [1.25, 1.29, 'mCrown', .02], [1.29, 99, 'mPurple', 0]],
  mbed: [[0, .12, 'mBase', .012], [.12, 2.55, 'mauve', 0], [2.55, 99, 'mTop', 0]],
  rbed: [[0, .12, 'mBase', .012], [.12, 2.55, 'rose', 0], [2.55, 99, 'mTop', 0]],
  guys: [[0, .13, 'cBase', .012], [.13, 2.58, 'cream', 0], [2.58, 2.62, 'cCrown', .025], [2.62, 99, 'cream', 0]],
  gbath: [[0, 1.3, 'tileW', 0], [1.3, 1.34, 'cCrown', .02], [1.34, 99, 'cream', 0]],
  cbed: [[0, .13, 'cBase', .012], [.13, 99, 'blueGrey', 0]],
  jbed: [[0, .13, 'cBase', .012], [.13, 99, 'tan', 0]],
  hall: [[0, .14, 'hBase', .012], [.14, .98, 'wains', 0], [.98, 1.04, 'hRail', .035], [1.04, 99, 'hPlaster', 0]],
  plain: [[0, 99, 'plain', 0]],
};
...
export function paintGeo(geo, hex) {
  tint.setHex(hex);   /* setHex reads sRGB and stores linear */
...
const bare = g => { const c = new THREE.BufferGeometry(); c.setAttribute('position', g.attributes.position.clone()); if (g.index) c.setIndex(g.index.clone()); return c; };

/* a wall from a to b, facing its room on the left of a -> b; the upper part can drop to a stub
   while the wall stands between the camera and that room */
export function buildWall(o) {
  const [x1, z1] = o.a, [x2, z2] = o.b, dx = x2 - x1, dz = z2 - z1, len = Math.hypot(dx, dz);
  const ux = dx / len, uz = dz / len;
  const th = o.t ?? .14, H = o.h ?? WALL_H;
  ...
  const addBox = (key, s0, s1, y0, y1, t, z) => {
    const L = s1 - s0, xc = s0 + L / 2, paint = PAINT[key], m = paint != null ? MAT.paint : MAT[key];
    for (const [k, a, b] of [['stub', y0, Math.min(y1, STUB_H)], ['upper', Math.max(y0, STUB_H), y1]]) {
      ...
      if (paint != null) paintGeo(geo, paint);
      proxy.push(bare(geo));
  ...
  const front = LAYERS[o.style || 'monica'], back = o.back ? LAYERS[o.back] : null;
  for (const [s0, s1, y0, y1] of spans) {
    ...
    for (const [a0, a1, key, extra] of front) {
      ...
      if (back) { const t = th / 2 + extra / 2; addBox(key, s0, s1, yy0, yy1, t, t / 2); }
      else addBox(key, s0, s1, yy0, yy1, th + extra, 0);
    }
    if (back) for (const [a0, a1, key, extra] of back) {
      const yy0 = Math.max(y0, a0), yy1 = Math.min(y1, a1, H);
      if (yy1 - yy0 < .006) continue;
      const t = th / 2 + extra / 2; addBox(key, s0, s1, yy0, yy1, t, -t / 2);
    }
    if (o.skin) addBox('wallSkin', s0, s1, y0, y1, .05, -(th / 2 + .025));
  }

  const capW = th + (o.skin ? .1 : .05), capZ = o.skin ? -.025 : 0;
  if (o.cap !== false) {
    const geo = new THREE.BoxGeometry(len + th, .06, capW);
    geo.translate(len / 2, H + .03, capZ); paintGeo(geo, PAINT.poche); proxy.push(bare(geo)); geo.translate(0, -STUB_H, 0);
    const list = buckets.upper.get(MAT.paint) ?? []; buckets.upper.set(MAT.paint, list); list.push(geo);
  }
...
/* the attached pieces with nothing to click or move merge into one group per wall */
export function mergeDecor(merge) {
  for (const w of walls) {
    const plain = w.attach.filter(a => {
      let ok = !a.userData.hotspot && a.parent === app.root;
      a.traverse(x => { if (x !== a && (x.userData.keep || x.userData.hotspot)) ok = false; });
      return ok;
    });
    if (plain.length < 2) continue;
    const g = new THREE.Group(); app.root.add(g);
    for (const a of plain) { g.attach(a); a.userData.keep = false; }
    w.attach = w.attach.filter(a => !plain.includes(a));
    merge(g); w.attach.push(keep(g)); g.userData.merged = true;
  }
}

...
function pick(x, y) {
  ...
  for (const h of hits) {
    ...
    if (!r) continue;
    const occ = ray.intersectObjects(walls.flatMap(w => [w.upper, w.stub]).filter(shown), true);
...
function setHover(info, x, y) {
  ...
  app.emit('tip', info && x != null ? { title: info.title, x, y } : null);
}

export function activate(info) {
  info.action?.();
  const text = typeof info.text === 'function' ? info.text() : info.text;
  app.emit('card', { title: info.title, text });
  app.emit('hotspot', info.title);
}

export function buildInteract() {
  const el = app.renderer.domElement;
  el.addEventListener('pointermove', e => {
    if (e.pointerType !== 'mouse' || app.mode === 'sitcom') return;
  ...
  el.addEventListener('pointerup', e => {
    if (!down || app.mode === 'sitcom') return;

...
export function buildParticles() {
  const rnd = seeded(13), N = 200, dot = TX.softDot();
  const base = new Float32Array(N * 3), ph = new Float32Array(N * 2);
  for (let i = 0; i < N; i++) {
    base[i * 3] = -8 + rnd() * 17.5; base[i * 3 + 1] = .2 + rnd() * 2.2; base[i * 3 + 2] = -2.6 + rnd() * 5.6;
  ...
  const anchor = app.steamAnchor ?? new THREE.Vector3(0, -9, 0);
